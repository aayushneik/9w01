...
def on_group_member_join(data):
	if data.comId==cidy:
		try:
			...
			subclient.send_message(chatId=data.message.chatId,message=f"""[BC]  ─━─━─━─͜͡─͜͡  ⭞ 🍮⃟⃟ૈ͙⃨ .❣️ ꪝꫀᥨᩮꪔꫀ .🍰᪶֞◞⃕  ‍  𝐭𝐨  ⏜ℜ❍ლℰ❍ ミ★KıN͜͡ɠdőм☩ ♡    ∩_∩
 （„• ֊ •„)♡
 ┏━━∪∪━━━ღ❦ღ━━┓
          𝔗𝔥𝔯𝔢𝔢                                                  #A𝓁𝓁✯
[BC]✾✾✾✾✾✾✾✾✾✾✾✾✾✾
...
➼ unblock""")
			...
			if x.lower()=="*client" and c==[]:
				try:
					subclient.send_message(chatId=data.message.chatId,message="""[cb]vicious didnt add that command yet, please PM/DM him""")
					print(f"Info requested by {data.message.author.nickname}")
				except Exception as e:
					print(e)
